...
void marcarMision(int idx, int digito) {
    ...
    cout << " Mision superada. Digito desbloqueado: " << digito << "\n";
...
void misionCapitulo3() {
    cout << "\n Mision Capitulo 3: Compiladores y toolchain\n";
    int puntos = 0; 
    string entrada;

    // 1) Texto
    cout << "1) Que extension suele usarse para un archivo fuente de C++? (ej: .cpp): ";
    cin >> entrada; normalizar(entrada);
    if (entrada == ".cpp" || entrada == ".cc" || entrada == ".cxx") puntos++;

    // 2) Texto
    cout << "2) En consola, ¿que comando de GCC compila normalmente C++? ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "g++") puntos++;

    // 3) Código
    cout << "3) Escribe UNA linea que imprima: Hola mundo\n> ";
    cin.ignore();
    getline(cin, entrada);
    ...
    // 5) Texto
    cout << "5) La compilacion ocurre ANTES de ejecutar el programa? (si/no): ";
    cin >> entrada; normalizar(entrada);
    ...
    else cout << " No se completo la mision. Intentalo de nuevo.\n";
}

void misionCapitulo4() {
    cout << "\n Mision Capitulo 4: Tu primer programa\n";
    int puntos = 0; 
    string entrada;

    cout << "1) Como se llama la funcion donde inicia un programa C++?: ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "main" || entrada == "main()") puntos++;

    cout << "2) Header tipico para usar cout: ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "<iostream>") puntos++;

    cout << "3) Escribe UNA linea valida que imprima \"UVG\" en consola:\n> ";
    cin.ignore();
    getline(cin, entrada);
    ...
    cout << "5) Que simbolo usas con cout para enviar datos a la salida? (escribelo): ";
    cin >> entrada;
    if (entrada == "<<") puntos++;

    cout << "6) Que inicia un comentario de una sola linea?: ";
    cin >> entrada;
    if (entrada == "//") puntos++;
...
void misionCapitulo5() {
    cout << "\n Mision Capitulo 5: Tipos de datos y variables\n";
    int puntos = 0; 
    string entrada;

    cout << "1) Tipo entero mas comun en ejemplos: ";
    cin >> entrada; normalizar(entrada);
    ...
    cout << "3) Escribe una declaracion que cree un entero x con valor 5 usando llaves {}:\n> ";
    cin.ignore();
    getline(cin, entrada);
    if (contiene(entrada, "int") && contiene(entrada, "x") && contiene(entrada, "{5}")) puntos++;

    cout << "4) Tipo que almacena un solo caracter: ";
    cin >> entrada; normalizar(entrada);
...
void misionCapitulo6() {
    cout << "\n Mision Capitulo 6: Exercises (practica basica)\n";
    int puntos = 0; 
    string entrada;

    cout << "1) Bucle que repite mientras una condicion sea verdadera: ";
    cin >> entrada; normalizar(entrada);
    ...
    cout << "3) Escribe una linea que declare una variable llamada nombre de tipo string:\n> ";
    cin.ignore();
    getline(cin, entrada);
    normalizar(entrada);
    ...
    cout << "5) Operador de asignacion (el de 'darle' un valor a una variable): ";
    cin >> entrada;
    if (entrada == "=") puntos++;

    cout << "6) endl imprime salto de linea y vacia el buffer? (si/no): ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "si" || entrada == "sí") puntos++;
...
void misionCapitulo7() {
    cout << "\n Mision Capitulo 7: Strings\n";
    int puntos = 0; 
    string entrada;

    cout << "1) Tipo estandar para cadenas de texto en C++: ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "string") puntos++;

    cout << "2) Header que normalmente se incluye para usar string (segun ejemplos): ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "<string>") puntos++;

    cout << "3) Operador tipico para concatenar strings: ";
    cin >> entrada;
    if (entrada == "+") puntos++;

    cout << "4) Que metodo te da el tamaño de un string? (length/size): ";
    cin >> entrada; normalizar(entrada);
    ...
    else cout << " No se completo la misión. Intentalo de nuevo.\n";
}

void misionCapitulo8() {
    cout << "\n Mision Capitulo 8: Loops y arrays\n";
    int puntos = 0; 
    string entrada;

    cout << "1) Loop comun cuando tienes un contador (i=0; i<n; i++): ";
    cin >> entrada; normalizar(entrada);
    ...
    cout << "3) Escribe una declaracion para un array de 5 enteros llamado a:\n> ";
    cin.ignore();
    getline(cin, entrada);
    normalizar(entrada);
    if (contiene(entrada, "int") && contiene(entrada, "a") && contiene(entrada, "[5]")) puntos++;

    cout << "4) En un array de tamaño 5, el ultimo indice es: ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "4") puntos++;
...
void misionCapitulo16() {
    cout << "\n Mision Capitulo 16: Statements y control\n";
    int puntos = 0; 
    string entrada;

    cout << "1) Palabra clave para una condicion: ";
    cin >> entrada; normalizar(entrada);
    ...
    cout << "4) Operador logico AND (ambas condiciones verdaderas): ";
    cin >> entrada;
    if (entrada == "&&") puntos++;
    ...
    else cout << " No se completo la mision. Inténtalo de nuevo.\n";
}

void misionCapitulo17() {
    cout << "\n Mision Capitulo 17: Const y constexpr\n";
    int puntos = 0; 
    string entrada;
    ...
    cout << "2) constexpr sugiere evaluacion en compilacion? (si/no): ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "si" || entrada == "sí") puntos++;

    cout << "3) Escribe una linea valida: const int x = 5;\n> ";
    cin.ignore();
    getline(cin, entrada);
    normalizar(entrada);
...
void misionCapitulo18() {
    cout << "\n Mision Capitulo 18: Exercises (condicionales)\n";
    int puntos = 0; 
    string entrada;
    ...
    cout << "2) Operador logico OR (una u otra condición): ";
    cin >> entrada;
    if (entrada == "||") puntos++;

    cout << "3) 'else if' es una construccion valida en C++? (si/no): ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "si" || entrada == "sí") puntos++;

    cout << "4) Escribe una condicion que evalue si n es par usando el operador % (solo la expresion):\n> ";
    cin.ignore();
    getline(cin, entrada);
    // aceptamos varias formas
    if (contiene(entrada, "%") && (contiene(entrada, "== 0") || contiene(entrada, "==0"))) puntos++;

    cout << "5) return termina la funcion actual? (si/no): ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "si" || entrada == "sí") puntos++;

    cout << "6) Puedes usar if anidados para decisiones multiples? (si/no): ";
    cin >> entrada; normalizar(entrada);
    if (entrada == "si" || entrada == "sí") puntos++;
...
void mostrarMenu() {
    cout << "\n==============================\n";
    cout << "  BUSQUEDA DEL TESORO (9 misiones)\n";
    cout << "==============================\n";
    cout << "Elige el capitulo/mision:\n";
    cout << " 3  -> Mision Cap. 3\n";
    cout << " 4  -> Mision Cap. 4\n";
    cout << " 5  -> Mision Cap. 5\n";
    cout << " 6  -> Mision Cap. 6\n";
    cout << " 7  -> Mision Cap. 7\n";
    cout << " 8  -> Mision Cap. 8\n";
    cout << " 16 -> Mision Cap. 16\n";
    cout << " 17 -> Mision Cap. 17\n";
    cout << " 18 -> Mision Cap. 18\n";
    cout << " 0  -> Salir\n";
    cout << "Opcion: ";
}
...
int main() {
    ...
    do {
        ...
        switch (opcion) {
            ...
            default: cout << "⚠️  Opción invalida. Intenta de nuevo.\n";
        }
